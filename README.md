...
Spring Boot는 `REDIS_HOST`, `REDIS_PORT` 환경변수로 Redis에 연결합니다. `application.yaml`에는 Redis 설정을 두지 않고, `application-local.yml`에서 로컬 Docker Compose 기준 기본값인 `localhost:6379`를 선언합니다.
