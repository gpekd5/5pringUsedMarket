...
* JWT 발급
* JWT 검증
* Refresh Token Redis Whitelist
* Access Token Redis Blacklist

## repository

인증 도메인에서 사용하는 Redis 저장소를 관리한다.
...
Redis Lock 접근 로직은 `coupon.repository`에 둔다.

예시

```text
CouponRedisLockRepository
```
...
Redis Pub/Sub 접근 로직은 `chat.repository`에 둔다.
