...
- `image_key`는 서버가 업로드 URL 발급 응답으로 반환한 `products/{uuid}.{jpg|jpeg|png}` 형식을 저장한다.
...
상품 이미지는 `ImageUploadController`에서 Presigned PUT URL을 발급받아 S3에 직접 업로드한 뒤 반환받은 `imageKey` 목록을 상품 등록·수정 요청의 `imageKeys` 필드로 전달해 관리한다.
상품 도메인은 `products/{uuid}.{jpg|jpeg|png}` 형식의 key만 저장하고, 상세/목록/검색 응답에서는 해당 key를 Presigned GET URL로 변환해 `imageUrls` 또는 `thumbnailUrl`로 반환한다.
